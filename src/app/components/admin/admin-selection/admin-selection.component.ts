...
export class AdminSelectionComponent {
      ...
      switch (this.model.toLowerCase()) {
        case "product":
          ...
          this.productService.getAllProducts().subscribe((data) => {
            this.items = data.map(item => ({ id: item._id, title: item.title }));
            this.displayedItems = this.items;
            this.selectedItems = new Array(this.displayedItems.length).fill(false);
          });
          break;
        case "cart":
          this.items = [];
  ...
  performSearch(keyword: string) {
    this.displayedItems = this.items.filter((item: { title?: string; id?: string }) => {
      if (item.title) {
